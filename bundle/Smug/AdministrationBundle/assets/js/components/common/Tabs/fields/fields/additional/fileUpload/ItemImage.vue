<template>
  <section class="h-full">
    <article
      ...
      tabindex="0"
      class="group hasImage w-full h-full rounded-md focus:outline-none focus:shadow-outline text-transparent hover:text-white shadow-sm"
    >
      <div class="flex flex-col pb-5 mb-5">
        ...
        <section
          class="flex flex-col w-full z-20 top-0 rounded-md text-xs break-words h-full"
        >
    ...
    <image-modal
      ...
      :image-data="fieldValue"
      @reaction="showImagePreview(false)"
    />
  </section>
</template>
<script>
...
export default defineComponent({
  ...
  methods: {
    showImagePreview(state) {
      this.showImageModal = state;
    },
    getImageSource() {
      if (!this.fieldValue.media) {
        return ImageService.getImagePath(this.fieldValue);  
      }
      return ImageService.getImagePathFromMedia(this.fieldValue.media);
    },
    getImageName() {
      if (!this.fieldValue.media) {
        return TextService.getOutput(this.fieldValue.file, 10);  
      }

      return TextService.getOutput(this.fieldValue.media.file, 10);
    },
    removeImage(index) {
