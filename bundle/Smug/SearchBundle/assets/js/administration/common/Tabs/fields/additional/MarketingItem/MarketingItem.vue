    ...
    <vue-collapsible :is-open="expanded === true">
      <div class="pt-1 pb-5 px-2 border-t border-gray">
        ...
        <field
          ...
          :base-id="baseId"
          :item-value="item.linkTarget"
          :field-config="{fromDomain: true}"
          :field-placeholder="'DETAIL_LINK'"
          @updateValue="setContent($event, 'linkTarget')"
        />
        ...
        <field
          ...
          :field-string="'Text'"
          :base-id="baseId"
          :item-value="item.headline"
          :field-config="{}"
          :field-placeholder="'HEADLINE'"
          @updateValue="setContent($event, 'headline')"
        />
        ...
        <field
          ...
          :field-string="'Text'"
          :base-id="baseId"
          :item-value="item.captionText"
          :field-config="{}"
          :field-placeholder="'CAPTION'"
          @updateValue="setContent($event, 'captionText')"
        />
        ...
        <field
          :edit-allowed="editAllowed"
          :field-string="'FileUpload'"
          :base-id="item.id"
          :item-value="item.images"
          :field-config="{
            deleteCall: '/be/api/smug/search/MediaMarketingItemAssociation/delete',
            uploadCall: '/be/api/media/image/upload',
            assignCall: '/be/api/smug/search/marketingItem/images/assign',
            assignAlbum: 'frontend'
          }"
          :field-placeholder="'IMAGE'"
          @updateValue="setContent($event, 'images')"
        />
      </div>
...
export default {
  ...
  methods: {
    ...
    setContent(content, key) {
      this.marketingItem[key] = content;
    },
    deleteItem() {
      ApiService.put('/be/api/smug/search/marketingItem/delete', this.marketingItem).then(result => {
