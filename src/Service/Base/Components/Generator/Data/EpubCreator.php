...
class EpubCreator
{
    ...
    public function AddPage(
        ...
        // If it's XHTML
        if ( $content ) {
            $this->pages[$this->key]['content'] = '<p style="font-size: 24px; text-align: center;">' . $title . '</p>' . $content;
        }
    ...
    public function CreateEPUB() {
        ...
        foreach( $this->pages as $key => $value ) {
            ...
            $value['content']  = $page_content . $this->removeUmlauts($value['content']);
    ...
    private function removeUmlauts(string $content): string {
