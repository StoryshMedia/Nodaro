...
namespace Smug\Core\Service\Base\Components\Preparer\Slugify;

/**
 * Class Slugify
 * @package Smug\Core\Service\Base\Components\Preparer\Slugify
 */
class Slugify
{
    const CHAR_MAP = [
    ...
    public static function slugify($text, $separator = '-', array $modifier = [])
    {
        $normalized = strtolower(strtr($text, $modifier + self::CHAR_MAP));
        $cleaned = preg_replace($unwantedChars = '/([^a-z0-9]|-)+/', $separator, $normalized);

        return (strlen(trim($cleaned, $separator)) > 200) ? substr(trim($cleaned, $separator), 0, 200): trim($cleaned, $separator);
    }
}
