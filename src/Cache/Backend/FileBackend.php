...
class FileBackend extends SimpleFileBackend implements FreezableBackendInterface, TaggableBackendInterface
{
    ...
    {
        if (!is_string($data)) {
            throw new Exception('The specified data is of type "' . gettype($data) . '" but a string is expected.', 1204481674);
        }
    ...
    public function findIdentifiersByTag($searchedTag)
    {
        ...
        $cacheEntryFileExtensionLength = strlen($this->cacheEntryFileExtension);
        for ($directoryIterator = ServiceGenerationFactory::createInstance(\DirectoryIterator::class, $this->cacheDirectory); $directoryIterator->valid(); $directoryIterator->next()) {
            if ($directoryIterator->isDot()) {
                continue;
            }
