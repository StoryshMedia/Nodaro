...
use Smug\Core\Entity\Connection\Query\BulkInsertQuery;
use Smug\Core\Entity\Connection\Query\Expression\ExpressionBuilder;
...
class Connection extends \Doctrine\DBAL\Connection implements LoggerAwareInterface
{
    ...
    public function __construct(array $params, Driver $driver, Configuration $config = null, EventManager $em = null)
    {
        ...
        $this->_expr = ServiceGenerationFactory::createInstance(ExpressionBuilder::class, $this);
    }
    ...
    {
        return ServiceGenerationFactory::createInstance(QueryBuilder::class, $this);
    }
    ...
    {
        $query = ServiceGenerationFactory::createInstance(BulkInsertQuery::class, $this, $tableName, $columns);
    ...
    public function prepareConnection(string $commands)
    {
        ...
        $this->prepareConnectionCommands = DataHandler::trimExplode(
            chr(10),
            str_replace(
                '\' . LF . \'',
                chr(10),
    ...
    private function ensureDatabaseValueTypes(string $tableName, array &$data, array &$types): void
    {
        $setAllTypes = $types === [];
        $tableDetails = $this->introspectTable($tableName);
    ...
    private function introspectTable(string $tableName)
    {
