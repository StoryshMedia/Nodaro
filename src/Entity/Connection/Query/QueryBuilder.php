...
class QueryBuilder
{
    ...
    public function __construct(
        ...
        $this->concreteQueryBuilder = ServiceGenerationFactory::createInstance(\Doctrine\DBAL\Query\QueryBuilder::class, $connection);
    }
    ...
    public function quoteArrayBasedValueListToIntegerList(array $values): string
    {
        ...
        $values = DataHandler::intExplode(',', implode(',', $values));
    ...
    public function quoteArrayBasedValueListToStringList(array $values): string
    {
        ...
        $values = DataHandler::trimExplode(',', implode(',', $values));
    ...
    private function throwExceptionOnNamedParameterForPreparedStatement(array $params): void
    {
        foreach ($params as $key => $value) {
            if (is_string($key) && !DataHandler::isInteger($key)) {
